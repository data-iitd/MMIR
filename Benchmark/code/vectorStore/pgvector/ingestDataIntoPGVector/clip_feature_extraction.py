...
# FOR blip2
# model, vis_processors, txt_processors = load_model_and_preprocess(name=f"{MODEL_NAME}_feature_extractor", model_type="pretrain", is_eval=True, device=device)
model, vis_processors, txt_processors = load_model_and_preprocess(name=f"{MODEL_NAME}_feature_extractor", model_type="ViT-B-32", is_eval=True, device=device)
...
for split in coco_dataset:
    for data in coco_dataset[split]:
        ...
        features_image = features_multimodal.image_embeds_proj
        data_toingest = {
            'table_name':f'image_embeddings_{MODEL_NAME}_image',
            'image_name':data['image_id'],
            'image_path':img_path,
            'embedding':features_image.reshape(-1).cpu().detach().numpy().tolist(),
            'other_metadata':{
                'caption' : caption
            }
        }
        ...
        features_text = features_multimodal.text_embeds_proj
        data_toingest = {
            'table_name':f'image_embeddings_{MODEL_NAME}_text',
            'image_name':data['image_id'],
            'image_path':img_path,
            'embedding':features_text.reshape(-1).cpu().detach().numpy().tolist(),
            'other_metadata':{
                'caption' : caption
            }
        }
...
# print(f"[STATS]:[{data['image_id']}]IngestionTime={time_end-time_start}",flush=True)
